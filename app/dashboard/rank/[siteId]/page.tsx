...
import { RankDetailUI } from "./RankDetailUI";
import Link from "next/link";
import { redirect, notFound } from "next/navigation";
import { ArrowLeft, TrendingUp } from "lucide-react";

export default async function RankSiteDetailPage({ params }: { params: Promise<{ siteId: string }> }) {
    ...
    <div className="min-h-screen" style={{ background: "var(--bg)" }}>
      <header className="sticky top-0 z-40" style={{ background: "rgba(10,10,10,0.8)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="mx-auto max-w-6xl px-6 sm:px-8 h-14 flex items-center justify-between">
          ...
          <Link href="/dashboard/rank" className="flex items-center gap-1.5 text-sm text-[var(--text-tertiary)] hover:text-[var(--text-primary)] transition-colors">
      ...
      <div className="flex" style={{ minHeight: "calc(100vh - 56px)" }}>
        ...
        <main className="flex-1 min-w-0 mx-auto max-w-3xl px-6 sm:px-8 py-8 space-y-6">
          ...
          <RankDetailUI siteId={siteId} accessToken={accessToken} />
        </main>
      </div>
    </div>
  );
