...
## 3.1. Telegram — `setWebhook` має включати `callback_query`

Instant Actions (кнопки "🛠 Замовити виправлення" / "Пізніше" під
critical issues з `/issues`) працюють через Telegram update-тип
`callback_query`, не `message`. Якщо `setWebhook` був викликаний з
явним параметром `allowed_updates` (наприклад `["message"]`, типове
початкове налаштування, коли ботом ще не планувались inline-кнопки) —
Telegram **не надсилатиме** `callback_query` апдейти взагалі, і
натискання кнопки в чаті користувача просто вічно "крутитиметься"
без жодної реакції серверу (без помилки в логах worker — апдейт
ніколи не долітає).

Якщо `allowed_updates` НЕ був заданий явно при попередньому виклику
`setWebhook` — усе вже працює, Telegram за замовчуванням шле всі типи.
Якщо був — потрібно повторно викликати `setWebhook` без обмеження
(чи з явним включенням `callback_query`):

```
https://api.telegram.org/bot<TOKEN>/setWebhook?url=<APP_URL>/api/telegram/webhook&allowed_updates=["message","callback_query"]
```

🔴 Стан невідомий — немає доступу перевірити поточну конфігурацію
`setWebhook` на боті напряму. Перевір після деплою: натисни кнопку
"🛠 Замовити виправлення" в Telegram і подивись, чи з'являється
спливаюче підтвердження "✅ Заявку надіслано" протягом кількох секунд.
