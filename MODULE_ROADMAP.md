...
(`0071_creator_canvas.sql`), `creatorHandler.ts` (worker),
`/dashboard/creator` — список дощок + `/dashboard/creator/[boardId]`
— сам canvas на `@xyflow/react` (нова залежність, встановлена цим
проходом). Один `node_type='embedded_editor'`: `ProjectEditorUI.tsx`
(вже наявний Sites-редактор) рендериться НАПРЯМУ як React-компонент
