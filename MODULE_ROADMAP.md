...
## Qorax Canvas — візуальний шар над усією платформою (поза чергою хвиль)

**Джерело:** продуктове бачення від Артема — не "аналог Figma", а
візуальне робоче середовище, де об'єкти всієї платформи (статті,
сторінки сайту, AI-агенти, email-кампанії, CRM-угоди) існують на
одному нескінченному полотні, з кнопкою "Open in Canvas" у кожному
модулі.

**Принципова відмінність від Qorax Mail (розділ вище) — і чому
Canvas НЕ можна планувати так само:** Mail самодостатній — можна
почати будувати паралельно хвилям 1-4, він не залежить від решти
платформи технічно. **Canvas — рівно навпаки: він майже не має
цінності в ізоляції.** Найсильніша ідея бачення ("будь-який об'єкт
Qorax — елемент Canvas", кнопка "Open in Canvas" у кожному модулі)
за визначенням вимагає, щоб ці модулі — Sites, CRM, Mail, AI-агенти
— вже існували, мали стабільні типи об'єктів і реальні дані
користувачів. Порожній Canvas без об'єктів для перетягування — це
просто ще один застосунок для малювання, яких за словами самого
Артема "вже багато". **Тому Canvas реалістично один з НАЙПІЗНІШИХ
великих напрямків, не один з ранніх — на відміну від Mail.**

### Технічний фундамент, який визначає все інше

**Ключове рішення ДО коду, серйозніше за все в Qorax Mail:**
нескінченне полотно з плавним pan/zoom, hit-testing для сотень
об'єктів, undo/redo, і (якщо потрібна командна робота на одному
Canvas одночасно) real-time мультиплеєрна синхронізація курсорів і
змін — це інженерний обсяг, порівнянний з тим, що в Figma/Miro
роблять окремі команди роками. **Будувати цей рушій з нуля для v1
нерозумно** — той самий принцип, що вже прийнятий для Mail
("не власний SMTP-сервер, а провайдер") і для платежів
("Stripe недоступний → LemonSqueezy, не власний процесинг"):
делегувати важку інфраструктурну частину. Реалістичний шлях —
готовий open-source SDK для нескінченного полотна в React
(наприклад tldraw — саме такий SDK, з підтримкою кастомних типів
вузлів, що напряму потрібно для "будь-який об'єкт Qorax = елемент
Canvas"), а Qorax-специфічна цінність — у ТИПАХ вузлів і в тому, що
вони пов'язані з реальними даними платформи, не в самому рушії
малювання.

**Real-time співпраця** (кілька людей на одному Canvas одночасно) —
окреме архітектурне рішення: Cloudflare Durable Objects (природний
вибір, платформа вже на Cloudflare Workers — не нова інфраструктура,
а розширення вже наявної) або сторонній сервіс синхронізації
(Liveblocks/Yjs-based). Не MVP — перша ітерація Canvas може бути
однокористувацькою (як звичайний редактор документа), співпраця в
реальному часі — друга.

### Шар 1 — Canvas Core (порожнє полотно, sticky notes, mind maps)
...
create table canvas_nodes (
  ...
  node_type text not null,     -- 'sticky_note' | 'text' | 'shape' | 'platform_object' | ...
  position_x real not null,
  position_y real not null,
  width real,
  height real,
  data jsonb not null default '{}'::jsonb, -- вміст sticky note, налаштування форми, і т.д.
  -- заповнюється лише для node_type='platform_object' (Шар 2 нижче):
  ref_table text,
  ref_id uuid,
  created_at timestamptz not null default now()
);
...
```
Це **структурно майже ідентично `kg_nodes`/`kg_edges` з хвилі 4**
(`ref_table`/`ref_id` — той самий м'який зв'язок на реальний рядок
іншої таблиці) — не випадково, дивись Шар 2 нижче.

**Крок 3 — UI:** нова секція `/canvas` — tldraw (чи еквівалент) як
рушій рендерингу/взаємодії, Qorax-шар зверху визначає кастомні типи
вузлів і панель "перетягнути об'єкт з платформи".

### Шар 2 — Platform Object Bridge: "будь-який об'єкт Qorax = елемент Canvas"

**Найсильніша й найоригінальніша частина бачення — і найприродніше
місце для перевикористання інфраструктури, а не побудови нової.**
`canvas_nodes` з `node_type='platform_object'` і заповненими
`ref_table`/`ref_id` — це РІВНО той самий патерн, що `kg_nodes` з
Knowledge Graph (хвиля 4, розділ 14). Замість двох паралельних
систем м'яких посилань на об'єкти платформи, є два реалістичні
варіанти:
1. `canvas_nodes.kg_node_id` — опційний прямий зв'язок на вже
   існуючий `kg_nodes.id`, коли об'єкт вже має вузол графа
   (сторінка/товар/лід/ключове слово — усе, що вже підключено до
   Knowledge Graph) — Canvas стає ВІЗУАЛЬНИМ РЕДАКТОРОМ того самого
   графа, не окремою паралельною сутністю.
2. Для об'єктів, яких Knowledge Graph ще не охоплює (email-кампанії
   з Qorax Mail, AI-агенти, dashboard-панелі) — `canvas_nodes`
   лишається джерелом істини, і, можливо, сам генерує відповідний
   `kg_nodes`-запис через `upsertNode()` — Canvas розширює граф,
   а не дублює.

Кнопка "Open in Canvas" у кожному модулі (Sites/CRM/Mail/AI) —
технічно: `upsertNode()`-подібний виклик (перевикористання функції
з `knowledgeGraph.ts`, не нова) при першому відкритті об'єкта на
Canvas, плюс збереження `position_x`/`position_y` там, де
користувач його розмістив.

### Шар 3 — Generative Builders (Sitemap/Database/Automation/AI Flow/Dashboard)

**Принципова відмінність від Шарів 1-2: тут Canvas не просто
показує чи організовує об'єкти — він їх СТВОРЮЄ** через кнопку
"Generate". Це для кожного білдера окремий, різний за складністю й
ризиком рівень:

- **Sitemap Builder → Generate** — найреалістичніший: структура
  сторінок на Canvas мапиться прямо на вже наявні
  `sitesBuilderHandler.ts` виклики (`project_pages` insert на
  кожен вузол sitemap) — пряме перевикористання Sites-конструктора,
  Canvas тут просто альтернативний UI для вже наявної дії.
- **Automation Builder → Generate** — теж реалістично: вузли/стрілки
  мапляться на вже наявну структуру `mail_automations.steps`
  (Qorax Mail) чи майбутні `ai_plans`/`ai_plan_steps` (AI Operating
  System, хвиля 4) — Canvas як візуальний редактор JSON-структури,
  яка й так потрібна цим системам.
- **AI Flow (SEO Agent → Content Agent → Translator → Publish →
  Analytics)** — це візуалізація `ai_plan_steps` з AI Operating
  System (хвиля 4, розділ 12), не нова концепція — Canvas дає їй
  візуальний редактор замість (чи на додачу до) списку кроків у
  Qorax AI Chat.
- **Database Builder — свідомо НАЙРИЗИКОВАНІШИЙ пункт, окрема
  пересторога.** Дозволити користувачам візуально проєктувати й
  ЗАСТОСОВУВАТИ реальні зміни схеми Supabase — це рівень ризику,
  порівнянний із тим, чому міграції зараз накочуються Артемом
  вручну через SQL Editor, а не автоматично з коду (лесон із
  "Ключові навчання" вище: конфлікти нумерації міграцій — вже
  болючий, добре знайомий клас проблем НАВІТЬ коли міграції пише
  один розробник). Дати це в руки кінцевому користувачу без
  ревʼю — реальний ризик пошкодження чужих production-баз даних.
  **Рекомендація: Database Builder на MVP — тільки візуальне
  проєктування (ER-діаграма) з експортом SQL-файлу, який користувач
  сам виконує де завгодно (не обов'язково в Qorax) — БЕЗ
  автоматичного застосування до реальної БД клієнта.** Перетворення
  на "застосувати одним кліком" — не MVP, і взагалі окреме
  обговорення безпеки, не суто технічна деталізація.
- **Dashboard Builder** — перетягування графіків/KPI/таблиць —
  вимагає окремого шару "віджетів, що вміють показувати дані
  довільного модуля" — не MVP-пріоритет, найпізніший з Шару 3.

### Шар 4 — Knowledge Graph Visualization

**Найдешевший можливий "вау"-момент з усього бачення Canvas** — дані
для цього ВЖЕ існують: `kg_nodes`/`kg_edges` з хвилі 4 (розділ 14)
вже наповнюються з Sites/CRM/Rank. Потрібен лише рендерер, що читає
`buildGraphContext()`-подібний запит (без текстового форматування
для AI-промпту, натомість — вузли й ребра напряму в
`canvas_nodes`/`canvas_edges` формат для tldraw) — жодної нової
бізнес-логіки збору даних, тільки візуалізація вже наявного графа.
**Це найкращий кандидат на MВП Canvas загалом** — не Canvas Core
"з нуля", а "показати граф, який AI Chat вже використовує, людям
візуально" — маленький, дешевий, і одразу демонструє саме ту
"найцікавішу частину", яку сам Артем виділив у баченні
("Компанія → Товари → Статті → Ключові слова → Ліди → Продажі").

### Шар 5 — Командна робота (коментарі, sticky notes від команди)

Пряме перевикористання `comments`/`activity_feed` з Team Workspace
(хвиля 4) — `target_type='canvas_node'`, той самий поліморфний
патерн, не нова система коментарів.

### Підсумок: залежності й чому Canvas іде пізніше, не раніше

```
Knowledge Graph (хвиля 4) ──────────→ Шар 4: KG Visualization
                                             (найдешевший MVP-кандидат)

Sites/CRM/Mail/AI-агенти (мають ВЖЕ  ──→ Шар 2: Platform Object Bridge
існувати зі стабільними об'єктами)        ("Open in Canvas" у кожному модулі)
                                             │
                                             ├─→ Шар 1: Canvas Core (рушій — tldraw/еквівалент)
                                             │
Team Workspace (хвиля 4) ──────────────→ Шар 5: командна робота
                                             │
AI Operating System (хвиля 4, розділ 12) ──→ Шар 3: AI Flow / Automation Builder
                                             (Sitemap Builder — не залежить, готовий рано;
                                              Database Builder — НЕ MVP, окреме рішення безпеки)
```
На відміну від Qorax Mail (розділ вище, можна починати незалежно й
паралельно хвилям 1-4), Canvas реалістичний лише після того, як:
(а) Knowledge Graph з хвилі 4 вже наповнений реальними даними
(інакше Шар 4 — найдешевший MVP-кандидат — показує порожній граф),
і (б) хоча б кілька модулів (Sites, CRM) стабільно `live` з
реальними об'єктами користувачів (інакше Шар 2 — "Open in Canvas" —
нема що відкривати). Технічно і продуктово це робить Canvas
кандидатом ПІСЛЯ хвилі 4, не поруч із нею, на відміну від Mail.
