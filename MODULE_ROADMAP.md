...
**Живу синхронізацію (Presence + save-notification) підключено
пілотно лише в Docs** — Sheets/Slides мають той самий готовий
патерн (`useLiveSync.ts`, спільний хук), підключення до двох інших
редакторів — невеликий, окремий наступний крок, не архітектурна
робота.
...
вважалось раніше. Наступні реалістичні кроки: (а) поширити живу
синхронізацію на Sheets/Slides, (б) Whiteboard — після того, як
Canvas Creator стабілізується, (в) Smart Blocks — після того, як
Smart Components в Creator реалізовані (той самий механізм,
переюзаний), (г) PDF Studio — окреме дослідження PDF-парсинг-
бібліотеки перед стартом.
