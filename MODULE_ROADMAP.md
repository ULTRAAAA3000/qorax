...
## Третя хвиля: Qorax Agents — оркестраційний шар

**Принципова відмінність від модулів 1-11:** Agents — це НЕ окремий
продукт з власними даними, а шар позиціонування і автоматизації
ПОВЕРХ уже збудованих модулів. "SEO Agent" не має власної бізнес-логіки
аудиту — він викликає той самий `seoChecker.ts`/`aiInsights.ts`, що і
зараз, просто за розкладом і з "особистістю" в подачі результату.
Тому Agents не можна будувати раніше модулів 1-9 (Rank, AI/Content,
Analytics, CRO, Commerce, Social) — кожен агент є персоною над вже
готовим модулем. Порядок в roadmap невипадковий: Agents свідомо
останні, бо це шар monetization/positioning, а не нова функціональність.

Виняток — **Support Agent**: єдиний агент без готового модуля-основи
(немає окремого "Support-модуля" в списку 1-11). Він частково новий
функціонал (чат-віджет на сайті клієнта), частково — розширення
CRM (модуль 7, збір лідів).

### Крок 1 — Схема БД
```sql
create table agents (
  id text primary key,                -- 'seo' | 'content' | 'translator' | 'analytics' |
                                       -- 'rank' | 'cro' | 'commerce' | 'social' | 'support' | 'ceo'
  name text not null,
  description text not null,
  underlying_module text,             -- посилання на модуль з розділів 1-9 ('rank', 'ai_content', ...)
...
create table agent_subscriptions (
  ...
  site_id uuid references sites(id) on delete cascade, -- null = на рівні організації (Social/CEO)
  schedule_cron text,                 -- '0 9 * * 1' (кожен понеділок о 9:00) — null = ручний запуск
...
create table agent_runs (
  ...
  summary text,                        -- людський виклад результату ("знайдено 18 помилок")
  raw_output jsonb,                    -- посилання на реальні дані (audit_id, ai_generation_id...)
...
create table agent_action_log (
  ...
  action_type text not null,           -- 'audit_triggered' | 'article_rewritten' | 'alert_sent'...
  target_table text,                   -- 'page_seo_audits' | 'ai_content_generations'...
  target_id uuid,
  created_at timestamptz not null default now()
);
```
`agents` — довідник, не залежить від організації (як `platform_modules`
для звичайних модулів). `underlying_module` явно документує залежність
кожного агента від реального коду модуля, щоб не виникло ілюзії, що
агент — це окрема система.

### Крок 2 — Worker-логіка
**Ключовий принцип: агент = тонка обгортка, не новий алгоритм.**
- `worker/src/lib/agentRunner.ts` — єдина точка входу
  `runAgent(agentId, subscriptionId)`, яка:
  1. читає `agents.underlying_module`
  2. викликає вже наявну функцію модуля (`runSeoAudit()` з
     seoChecker.ts для SEO Agent, `generateContent()` з
     contentGeneration.ts для Content Agent, `syncRankQueries()` для
     Rank Agent і т.д. — **нових алгоритмів немає, тільки нові виклики
     наявних**)
  3. списує `credit_cost_per_run` з `ai_credits` (модуль 2)
  4. формує `summary` людською мовою через короткий Gemini-виклик
     ("опиши результат у 2 реченнях для власника бізнесу")
  5. пише `agent_runs` + `agent_action_log`
  6. надсилає сповіщення (переюзати `dispatchAlert()`)
- Розклад (`schedule_cron` на кожну підписку) виконується НЕ окремим
  cron-тригером на кожного агента (Cloudflare Workers cron — обмежена
  кількість тригерів, і вони керуються вручну через Dashboard, а не
  per-запис в БД). Замість цього: один вже наявний щоденний/щогодинний
  cron перевіряє таблицю `agent_subscriptions` де
  `schedule_cron` "настав" (парситься бібліотекою cron-парсера в
  Worker) і викликає `runAgent()` для кожного, кому час
- **CEO Agent** — не окремий виклик модуля, а агрегатор:
  `worker/src/lib/ceoAgentSummary.ts` читає останні `agent_runs` всіх
  інших агентів організації за період і одним Gemini-викликом
  синтезує загальний звіт ("SEO Agent знайшов X, Content Agent
  переписав Y, тому рекомендується Z"). Це єдиний агент, що читає
  ВИХІД інших агентів, а не сирі дані модулів
- **Support Agent** — єдиний новий функціонал: легкий чат-віджет
  (JS-сніпет, як CRO-сніпет з модуля 9) на сторінці Sites (модуль 4)
  або зовнішнього сайту клієнта. Переюзовує `chatHandler.ts`
  інфраструктуру (та сама, що Qoraxus AI-чат), новий системний промпт
  "відповідай як менеджер підтримки [назва бізнесу клієнта]", ескалація
  до людини — просто позначка "потрібна людина" + запис ліда в CRM
  (модуль 7, `crm_contacts` з `source='support_agent'`)
- Ендпоінти: `POST /api/agents/:id/subscribe`,
  `PATCH /api/agents/subscriptions/:id` (розклад/увімкнено),
  `POST /api/agents/:id/run-now` (ручний запуск),
  `GET /api/agents/runs?organization_id=` (стрічка подій для UI)

### Крок 3 — UI
`app/dashboard/agents/page.tsx` — картки агентів (як "співробітники"):
іконка, назва, опис, статус (активний/вимкнений), кнопка "запустити
зараз", розклад. `app/dashboard/agents/feed/page.tsx` — стрічка подій
у стилі команди: "SEO Agent знайшов 18 помилок · 2 год тому",
"Content Agent переписав 4 сторінки · вчора" (читає `agent_runs`).
CEO Agent — окрема картка зверху стрічки з тижневим зведенням.
Support Agent — окремий екран налаштування чат-віджету (вигляд,
привітання) + вкладка "розмови" (читає ліди з CRM).

### Крок 4 — Обмеження по тарифу
Монетизація **не через окремі ai_credits-цифри в цьому документі**
(конкретні числа — рішення на етапі запуску, не частина технічного
плану), а через дві прості осі, які вже мають готову інфраструктуру:
- Кількість одночасно активних `agent_subscriptions` на тариф
  (Free = 1 агент, вищі тарифи = більше/всі) — перевірка при
  `POST /api/agents/:id/subscribe`, аналогічно лімітам інших модулів
- Кредити на запуск (`credit_cost_per_run`) списуються з єдиного пулу
  `ai_credits` (модуль 2) — **агенти НЕ отримують окремий пул кредитів**,
  бо кожен запуск агента технічно є викликом того самого модуля, який
  вже монетизується кредитами
- Автоматичний розклад (`schedule_cron`) — фіча вищих тарифів; на
  нижчих тарифах доступний тільки ручний запуск (`run-now`)

### Крок 5 — Готовність до "live"
MVP не вимагає всіх 10 агентів одразу. Мінімальний робочий набір:
**SEO Agent + Content Agent + Rank Agent** (три модулі, що вже готові
— Rank, AI/Content, і аудит вже існує окремо від модулів 1-4) +
проста стрічка подій без CEO-агрегації. Кожен наступний агент
додається без нової міграції — просто новий рядок у `agents` і нова
гілка в `agentRunner.ts`, коли відповідний модуль (Analytics/CRO/
Commerce/Social) готовий. CEO Agent і Support Agent — окремі кроки
після того, як є хоча б 3-4 працюючих агенти, чий вивід є що
агрегувати.

**Оцінка обсягу:** технічно малий шар (4 таблиці, 1 раннер, 1
парсер розкладу), але з жорсткою залежністю від готовності модулів
1-9 — це причина, чому Agents свідомо в кінці, а не на початку
roadmap. Основна складність не в коді, а в позиціонуванні/UX
("співробітники", а не "функції") і в написанні промптів для
человекочитаемых summary кожного запуску.
...
Хвиля 3:           Qorax Agents (оркестрація поверх хвиль 1-2)
```
Agents — завжди останні, бо кожен агент є персоною над вже готовим
модулем; будувати агента раніше за модуль, який він обгортає,
