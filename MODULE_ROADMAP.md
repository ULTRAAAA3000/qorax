...
**Підтверджено на практиці той самий патерн, що вже застосований для
Qorax Creator (розділ нижче):** топ-левел роут (`/mail`, не
`/dashboard/mail`), власний UI без Dashboard-каркасу
(`PlatformSidebar`/`platform_modules`), той самий домен. Основний
вхід у кожен продукт — спільний лендінг (Артем робить окремо);
переходи між продуктами — конкретні місця, не хедер, окреме пізніше
рішення. Дані лишаються спільними через `organization_id`, коли
Mail-акаунт прив'язаний до організації (див. відкрите питання про
автентифікацію нижче) — той самий принцип, що вже дозволяє Creator
редагувати ту саму `project_pages`, що Sites-конструктор у
Dashboard, без дублювання запису.
...
**Статус: Website Mode MVP реалізовано (перший крок з "Порядку
реалізації" в кінці цього розділу).** `canvas_boards`/`canvas_nodes`
(`0071_creator_canvas.sql`), `creatorHandler.ts` (worker), `/creator`
— список дощок + `/creator/[boardId]` — сам canvas на `@xyflow/react`
(нова залежність, встановлена цим проходом).

**Архітектурне уточнення від Артема (важливо, легко зрозуміти
неправильно):** Creator — ОКРЕМИЙ топ-левел продукт екосистеми,
той самий рівень, що сам Dashboard і майбутній Mail — НЕ модуль
у Dashboard-сайдбарі серед CRM/Commerce/Team Workspace. Перша версія
цього проходу помилково додала Creator у `platform_modules` як
"ще один модуль" — виправлено: `/creator`, не `/dashboard/creator`,
без `PlatformSidebar`/Dashboard-каркасу, без запису в
`platform_modules`. Дані (canvas_boards, organization_id) лишаються
СПІЛЬНИМИ з рештою платформи — саме це дозволяє редагувати ту саму
`project_pages`, що Sites-конструктор у Dashboard, без дублювання
запису. Основний вхід у кожен продукт — майбутній лендінг (Артем
робить окремо); прямі переходи між продуктами з хедера — теж
свідомо НЕ додано цим проходом (пізніший крок, конкретні місця
переходів Артем визначить окремо).

Один `node_type='embedded_editor'`: `ProjectEditorUI.tsx` (вже
наявний Sites-редактор) рендериться НАПРЯМУ як React-компонент
усередині canvas-вузла — не iframe (план описував iframe для Live
Objects, окремого пізнішого пункту; тут пряме вбудування точніше
виконує вимогу "той самий редактор, не переписаний вдруге", раз
компонент вже самодостатній). `canvas_edges` навмисно не додано
(Website Mode не потребує зв'язків між вузлами).

**Diagram Mode / KG Visualization — реалізовано (другий крок).**
`/creator/graph` — read-only візуалізація вже наявного Knowledge
Graph (`kg_nodes`/`kg_edges`, той самий шар, що вже споживає AI Chat
через `buildGraphContext()`) на `@xyflow/react`. Без окремого
`canvas_boards`-запису — граф один на організацію, не "дошка", яку
хтось розкладає вручну. Простий концентричний layout по
`node_type` (не dagre/елк — нова залежність не виправдана для
MVP-візуалізації, яку не редагують). Побічно виявлено й виправлено:
`addEdge()` ніде не викликався (граф був тільки вузли без ліній) —
додано 2 симетричні виклики (`crmHandler.ts`/`rankHandler.ts`):
лід ↔ ключові слова того самого сайту (`site_id`, гарантовано коректне
співставлення, не URL-евристика), обмежено 3 найновішими вузлами з
кожного боку, щоб зв'язок лишався змістовним, не розмивався у зірку.
...
**Components / Brand Kit — реалізовано (четвертий крок).** Схема
взята практично дослівно з плану (`0075_creator_components_brand_kit.sql`
— `creator_brand_kits` один запис на організацію, `creator_components`
з `content` у ТОМУ САМОМУ block-JSON форматі, що `project_pages.content`
— не новий формат "компонента"). `/creator/components` — Brand Kit
форма (лого/кольори/tone of voice) + бібліотека компонентів
(створення/видалення, категорії ті самі, що `BLOCK_TYPES` у
`ProjectEditorUI.tsx`). Схема готова під Marketplace одразу
(`organization_id` nullable = системний компонент, `is_marketplace`)
— сам Marketplace UI/логіка НЕ цей прохід, як план і відкладає його
явно ("Developer Mode, History, Multiplayer, Marketplace" — пізніше).
`tone_of_voice` зберігається, але інтеграція з Content Agent
(використання бренд-тону при AI-генерації текстів) — НЕ цей прохід,
тільки поле для майбутнього споживання.

**Smart Components — реалізовано (п'ятий крок).**
`canvas_nodes.bound_ref_table`/`bound_ref_id`/`field_bindings`
(`0079_creator_smart_components.sql`, дослівно схема з плану) — MVP
тільки для `products` (Commerce), доводить сам механізм "живого
зв'язку", не одразу підтримує довільну таблицю. Ключове: рендер
резолвиться в `handleBoardDetail` ПРИ КОЖНОМУ `GET` дошки
(`resolveSmartComponentData` у `creatorHandler.ts`), не при створенні
вузла — значення в `canvas_nodes.data` НЕ джерело істини для таких
вузлів, ціна на картці оновлюється сама, коли товар змінюється в
Commerce, без дії користувача над карткою. Whitelist
(`SMART_COMPONENT_ALLOWED_TABLES`) — той самий принцип безпеки, що
`LIVE_EMBED_ALLOWED` для Live Objects: без нього `bound_ref_table`
(вільний text) дозволив би читати ДОВІЛЬНУ таблицю бази.
`field_bindings` фіксований для MVP (`{title, price_label, image}`),
не редагується користувачем — довільний конструктор мапінгів
окрема, більша задача. UI — новий тип вузла `SmartComponentNode` у
`BoardCanvasUI.tsx`, двоетапний вибір у меню (проєкт → товар, той
самий реальний зв'язок даних Commerce, не штучне спрощення).

**AI Creator — реалізовано (шостий, останній крок ближньої
послідовності).** План описує дві частини: (1) генерація сторінки —
УЖЕ працює в Creator без жодного нового коду, бо Website Mode
рендерить `ProjectEditorUI.tsx` напряму, а той компонент вже має AI-
генерацію (`handleProjectPageAiGenerate`, окремий, раніший прохід).
(2) **AI Collaboration** ("переделай этот блок"/"сделай стиль как
Apple") — цей прохід додав точковий Gemini-виклик над одним
обраним `creator_components.content` (єдине місце з текстовим
content зараз — Website Mode/Live Objects/Smart Components не мають
тексту, який мало б сенс "переробляти"). Той самий принцип, що вже
прийнятий для Chat AI Actions (хвиля 3): `handleComponentRewrite`
ПОВЕРТАЄ прев'ю, НЕ зберігає автоматично — застосування (`PATCH`)
окремий, підтверджуючий крок користувача. Генерація зображень
свідомо НЕ вирішена (план явно каже "окреме рішення постачальника,
не вирішено цим записом").

Це завершує ближню послідовність реалізації Creator (Website Mode →
Diagram Mode → Live Objects → Components/Brand Kit → Smart
Components → AI Creator). Наступні кроки за планом — Email/
...
## Qorax Office — окремий продукт екосистеми (MVP реалізовано, повний Notion/Word-паритет — ні)

**Джерело:** продуктове бачення від Артема, отримано як текстовий
документ з повним описом фіч. **Статус оновлено: MVP-пріоритет із
самого документа Артема — реалізовано ПОВНІСТЮ, включно з
поглибленнями поверх нього. Повний перелік фіч з розділу нижче —
частково: те, що сам документ явно відніс до "пізніших ітерацій,
не MVP", свідомо не робилось.** (Попередній запис цього розділу
стверджував "БЕЗ коду" — застарів, код вже є, оновлено після
кількох проходів реалізації; детальний реєстр — у новому підрозділі
"Стан реалізації" нижче.)

**Архітектурне рішення, яке фактично прийняте кодом, а не лише
обговоренням:** розділ нижче (і "Наступний крок" в кінці) описував
Office як окремий домен/деплой. Реалізовано інакше — за прецедентом
Qorax Creator: `/office` всередині ОДНОГО Next.js застосунку,
спільна Supabase-сесія й `organization_id`, без окремого входу.
Причина — той самий принцип, що вже спрацював для Creator: спільна
сесія (кешування входу між продуктами, вже перевірене) і
відсутність потреби купувати/налаштовувати окремий домен ДО того,
як продукт довів свою цінність. Перехід на справді окремий домен —
можливий пізніше, без переписування вже готового коду (той самий
Next.js застосунок можна задеплоїти на власний домен, якщо/коли
Артем вирішить, що Office готовий бути самостійною точкою входу).

**Критична відмінність від усього вище в цьому документі:** все, що
описано в розділах 1-16 (Rank, Sites, CRM, Qorax AI, Benchmarking
тощо) — це модулі однієї платформи Qorax (яка в цій системі
продуктів тепер називається **Qorax Business** — те, що зараз є
`/dashboard` і всіма піддиректоріями всередині нього). Qorax Office —
не модуль `/dashboard`, не пункт у `platform_modules`/sidebar, а
окремий продукт зі своєю точкою входу (`/office`).

**Концепція екосистеми з чотирьох продуктів (Business + Creator +
Mail + Office):** Business, Creator і Mail вже зафіксовані в плані
раніше (окремими проходами/документами, не в цьому розділі — не
дублюється тут). Кожен продукт цінний окремо, але підсилює інші при
спільному використанні: дизайн з Creator потрапляє в документи
Office, документи Office відправляються через Mail, дані й звіти в
Office підтягуються з Business.

### Що таке Qorax Office

Не аналог Word чи Google Docs один в один (навмисно — "копіювати
Word — нескінченний проєкт"). AI-простір для щоденної офісної роботи
бізнесу: документи, таблиці, презентації, файли — з акцентом на AI,
що робить основну роботу, а не на паритет фіч з Microsoft Office.

**Продукти всередині Office:**
- **Docs** — редактор документів (заголовки, списки, таблиці,
  зображення, відео, код-блоки, цитати, callout, формули, чек-листи,
  вкладення) + **AI Writer**, що самостійно збирає готовий документ
  за текстовим запитом (структура → таблиці → оформлення →
  зображення → перевірка помилок), і **Smart Blocks** — "живі" блоки
  (Task/Meeting/Chart/CRM/Analytics/AI/Calendar/Checklist/Table/
  Code/Alert), що відрізняються від звичайного тексту тим, що
  підтягують дані з інших продуктів екосистеми
- **Sheets** — таблиці з AI-генерацією ("покажи прибуток по клієнтах
  за 6 місяців" → готова таблиця), фільтри, діаграми, CSV/Excel
  імпорт-експорт, формули, Dashboard
- **Slides** — презентації з AI ("створи презентацію для інвестора"
  → структура + дизайн + графіки + зображення + іконки)
- **Whiteboard** — окремий режим для мозкового штурму/схем/
  планування, використовує вже наявний Canvas Creator (Qorax
  Creator, не новий рушій)
- **Notes** — швидкі нотатки (текст, зображення, голос, чек-листи)
- **PDF Studio** — об'єднання/розділення/підпис/стиснення/
  конвертація/редагування тексту/витяг зображень з PDF
- **Templates** — бібліотека (договір, рахунок, комерційна
  пропозиція, резюме, інструкція, презентація, звіт, план проєкту,
  SOP, політика компанії)
- **AI Assistant** — працює всюди в Office ("зроби документ
  коротшим", "додай таблицю", "переклади", "зроби презентацію з
  документа")
- **Charts** — побудова графіків з таблиці/CRM/Business/CSV
- **Workspace** — організація документів по проєктах/папках/
  просторах/обраному/останніх
- **Universal Search** — семантичний пошук ("знайди договір з Acme,
  де була знижка 20%") — розуміння змісту, не лише назви файлу
- **Version History** — збереження кожної версії, порівняння,
  відновлення
- **Collaboration** — коментарі, пропозиції, режим перегляду,
  історія змін, спільне редагування в реальному часі
- **Export/Import** — PDF, DOCX, XLSX, PPTX, CSV, Markdown, HTML

**AI Workflows (наскрізні сценарії через продукти екосистеми):**
"напиши договір → зроби PDF → створи лист → відправ клієнту → створи
задачу, якщо не відповість за тиждень" — весь ланцюжок Office → Mail
→ Business(CRM/Tasks) в одному запиті, автоматично.

**Монетизація (ідеї з документа, не остаточні):** кількість
документів, обсяг сховища, AI-кредити, спільна робота, історія
версій, експорт у преміальні формати, корпоративні шаблони.

### Що свідомо НЕ робити на старті (з самого документа Артема)

Явна засторога від Артема: не намагатись одразу замінити Microsoft
Office — задача занадто велика. Пріоритет для MVP, якщо/коли
розробка почнеться:
- зручний редактор документів;
- AI, що реально економить час (не косметика);
- презентації;
- прості таблиці;
- робота з PDF;
- шаблони документів;
...
Whiteboard, Universal Search, повний набір Smart Blocks, глибокі AI
Workflows — пізніші ітерації, не MVP.

### Стан реалізації (оновлено після кількох проходів коду)

**MVP-пріоритет Артема (7 пунктів вище) — усі 7 реалізовано:**
- ✅ Зручний редактор документів — Docs, 4 типи блоків (paragraph/
  heading/bullet_list/checklist) + зображення (URL-based)
- ✅ AI, що реально економить час — AI Writer (Docs), AI Table
  Generator (Sheets), AI Slide Generator (Slides) — усі три
  переюзують один Gemini-пайплайн (`callGemini`), не три окремі
- ✅ Презентації — Slides, той самий блочний рушій, що Docs,
  drag-переупорядкування слайдів, Present-режим (fullscreen)
- ✅ Прості таблиці — Sheets, формули SUM/AVERAGE/COUNT, форматування
  клітинок (жирний/колір/числовий формат), діаграми (bar/line/pie,
  ручний SVG), multi-cell paste
- ✅ Робота з PDF — експорт Docs/Slides у PDF (jsPDF+html2canvas,
  клієнтський рендеринг для коректної кирилиці); **НЕ** повний PDF
  Studio (merge/split/sign/compress довільних файлів користувача —
  див. "Свідомо не зроблено" нижче)
- ✅ Шаблони документів — 5 системних (договір/рахунок/КП/план
  проєкту/SOP) + "зберегти власний як шаблон"
- ✅ Спільна робота — Presence (хто зараз дивиться документ,
  Supabase Realtime) + жива синхронізація при збереженні (пілотно в
  Docs); **НЕ** посимвольне одночасне редагування (CRDT) — див. нижче

**Плюс CSV та .xlsx імпорт/експорт у Sheets** (не було явно в
MVP-списку, додано як природне розширення "простих таблиць";
`exceljs`, не `xlsx`/SheetJS — той пакет з npm має 2 high-severity
вразливості САМЕ в парсингу файлів, задокументовано в
EXECUTION_PLAN.md).

**Свідомо НЕ зроблено (з повного переліку фіч вище — те, що сам
документ Артема відносить до "пізніших ітерацій"), з причиною:**
- **Whiteboard** — мав перевикористовувати Canvas Creator; Creator
  сам ще в процесі реалізації окремим паралельним потоком, Office
  Whiteboard логічно чекає на нього
- **Notes** (швидкі нотатки) — не MVP-пріоритет, окремий, менший
  режим від Docs
- **PDF Studio повністю** (merge/split/sign/compress/конвертація
  довільних PDF) — вимагає PDF-парсинг-бібліотеки, сумісної з
  Cloudflare Workers runtime (WASM, не Node.js-специфічна) АБО
  клієнтської бібліотеки з іншим рівнем довіри до вразливостей, ніж
  jsPDF (яка тільки ГЕНЕРУЄ PDF, не парсить довільні чужі файли) —
  не досліджено технічно
- **Smart Blocks** (Task/Meeting/Chart/CRM/Analytics/AI/Calendar) —
  "живі" блоки з даними інших продуктів — концептуально те саме, що
  Smart Components з довгострокового бачення Creator (`bound_ref_table`/
  `bound_ref_id`), не дубльовано двічі — коли Creator реалізує Smart
  Components, Office може переюзати той самий механізм
- **Workspace** (проєкти/папки/простори) — зараз усі документи
  одного типу в одному плоскому списку на організацію, без ієрархії
- **Universal Search** — семантичний пошук за змістом, не назвою —
  не почато
- **Version History** — немає збереження попередніх версій/
  порівняння/відновлення
- **Повне одночасне редагування (CRDT)** — детально обґрунтовано
  вище (запис "жива синхронізація при збереженні"): ризик
  "курсор стрибає" для plain-текстових полів без Yjs/ProseMirror-
  подібного біндингу, свідомо не робилось без пропорційної якості
  перевірки
- **Export/Import DOCX/PPTX/Markdown/HTML** — лише PDF (Docs/Slides)
  і CSV/XLSX (Sheets) реалізовано з переліку форматів
- **AI Workflows наскрізь через продукти** ("напиши договір → PDF →
  лист → CRM-задача") — вимагає Mail і глибшої інтеграції з Business,
  обидва самі ще не на тому рівні готовності

**Живу синхронізацію (Presence + save-notification) підключено до
всіх трьох редакторів** — Docs, Sheets, Slides (`useLiveSync.ts`,
спільний хук). Для Slides по дорозі виправлено реальний ризик
крашу fullscreen Present-режиму (якщо хтось інший зберігає
презентацію просто під час показу) — `isEditing()` враховує і
активний показ, не лише фокус в інпуті.

### Залежності від решти екосистеми

Office технічно спирається на продукти, які мають існувати раніше:
- **Creator** — Whiteboard навмисно перевикористовує Canvas Creator,
  не новий рушій з нуля
- **Mail** — інтеграція "документ → лист → відправка"
- **Business** (поточний `/dashboard`) — Smart Blocks
  (CRM/Analytics/Calendar) і AI Workflows тягнуть дані з модулів
  хвиль 1-3 цього документа (Rank/Analytics/CRM/Qorax AI)

Тому Office логічно стартує не раніше, ніж Business (хвилі 1-3) і
базові версії Creator/Mail вже існують — той самий принцип
"надбудова над готовим", що і хвиля 4 вище.

**Наступний крок (переглянуто):** MVP-реалізація вже відбулась
всередині поточного `qorax`-застосунку (`/office`), не окремим
деплоєм — питання "окремий домен/деплой чи monorepo" залишається
відкритим на майбутнє (якщо/коли Артем захоче Office як самостійну
точку входу з власним доменом), а не блокером для старту, яким
вважалось раніше. Наступні реалістичні кроки: (а) Whiteboard —
після того, як Canvas Creator стабілізується, (б) Smart Blocks —
після того, як Smart Components в Creator реалізовані (той самий
механізм, переюзаний), (в) PDF Studio — окреме дослідження
PDF-парсинг-бібліотеки перед стартом. Жива синхронізація вже
поширена на всі три редактори — не в цьому списку.

---

## Qorax Browser — окремий продукт екосистеми (концепція, не модуль платформи)

**Джерело:** продуктове бачення від Артема, п'ятий і останній продукт
екосистеми (після Business, Mail, Creator, Office). **Статус:
зафіксовано як напрямок на майбутнє, БЕЗ коду** — той самий рівень
фіксації, що Office вище: структурований запис, щоб не загубити
концепцію між сесіями, жодна реалізація цим проходом не починалась.

**Найважливіше застереження від самого Артема:** свідомо НЕ назва
"Browser" як єдине позиціонування — очікування "ще один Chrome"
неправильне. Робоча назва концепції — **Workspace Browser**: браузер
не для перегляду інтернету, а для роботи з ним. Не намагається
замінити Chrome технічно — використовує сучасні браузерні технології
як основу (той самий принцип "не будувати з нуля те, що вже
вирішене", що прийнятий для Canvas Core в Creator і email-інфраструктури
в Mail), а зверху додає AI-шар і глибоку інтеграцію з рештою
екосистеми.

**Точка входу:** окремий застосунок (десктопний браузер), не
піддиректорія `/dashboard`, не веб-сторінка — за масштабом ідеї це
принципово окрема категорія програмного забезпечення, ще більш
відокремлена від Business, ніж Mail/Creator/Office (ті хоч і окремі
продукти, але веб-застосунки на своїх доменах).

### Головна ідея

Будь-який сайт в інтернеті — не вкладка, а **об'єкт**: щось, що
браузер розуміє, аналізує, і може передати в інші продукти
екосистеми одним кліком. Головний екран — не вкладки, а **Workspace**
зі списком відкритих сайтів + бічна панель з AI/History/Collections/
Projects і прямими посиланнями на Business/Creator/Mail/Office.

### Ключові можливості

- **AI Sidebar** — присутня на будь-якій сторінці, розуміє контекст
  поточного сайту. Приклади команд з документа: "що це за сайт?",
  "зроби SEO аудит" (дані летять у Business), "зроби схожий дизайн"
  (макет у Creator), "створи лист на основі цієї сторінки" (шаблон у
  Mail), "зроби комерційну пропозицію" (документ в Office) — AI
  Sidebar тут є фактичним UI-мостом між Browser і рештою чотирьох
  продуктів
- **Collections** — "вбивця закладок": проєкт (наприклад "Інтернет-
  магазин одягу") збирає конкурентів/референси/статті/зображення/
  відео/ідеї в одному місці, а не розрізненими закладками
- **Smart Capture** — виділив будь-що на сторінці (hero-блок, текст,
  email, товар) → напряму в Creator/Office/Mail/Business відповідно
  до типу контенту
- **Site Inspector** — AI показує технічний і дизайн-профіль сайту:
  шрифти, кольори, зображення, компоненти, SEO, швидкість,
  технології/CMS/Framework, аналітичні сервіси на сторінці
- **One Click Actions (ПКМ-меню)** — Analyze SEO / Save to Project /
  Create Design / Generate Email / Translate / Summarize / Export
  PDF / Create Report / Add Task — усі одразу спрямовані у відповідний
  продукт екосистеми
- **AI Compare** — порівняння двох відкритих сайтів (свій сайт vs
  конкурент → різниці → рекомендації)
- **Reading Mode** — не просто чистий текст, а AI-стислий зміст,
  витягнуті факти, автоматичні нотатки
- **Research Mode** — агентний режим: AI сам відкриває N сайтів,
  вивчає їх, формує звіт, додає результат у Business — фактично
  той самий патерн, що Agents у Qorax AI (хвиля 3), але керує
  вкладками браузера, а не внутрішніми даними платформи
- **Visual Search** — виділена картинка → пошук джерела, схожих
  зображень, автоматична палітра, автоматичний SVG, усе одразу в
  Creator
- **Component Extractor** — найамбітніша фіча за оцінкою самого
  Артема ("може стати легендарною"): виділений компонент чужого
  сайту (наприклад Pricing-блок) → "Extract Component" → Creator
  отримує РЕДАГОВАНУ візуальну основу для натхнення, явно НЕ копію
  чужого HTML/CSS (важливо для юридичної чистоти — це джерело
  натхнення, не клонування)
- **Website Timeline** — як сайт виглядав раніше, AI показує зміни
...
- **Deep Search** — пошук по інтернету з AI, що сам підбирає
  приклади за складним природномовним запитом (не проста видача
  посилань)
- **AI Memory** — браузер пам'ятає, що вже вивчено, які сайти
  проаналізовано, які ідеї збережено
- **Marketplace** — встановлювані AI Agents/SEO Tools/Creator
  Plugins/Office Plugins/Mail Plugins

**Монетизація (ідеї з документа, не остаточні):** безкоштовний
браузер як база, Pro AI, хмарна синхронізація робочих просторів
(майбутнє), спільні проєкти, розширені AI-агенти, корпоративні
функції.

### Зв'язок з рештою екосистеми

Browser описаний Артемом як **вхідна точка**, що сама по собі нічого
не зберігає постійно — вся цінність у передачі даних в інші чотири
продукти:
```
Інтернет → Browser → Creator → Business → Mail → Office
```
Це робить Browser технічно останнім і найдорожчим з п'яти продуктів
екосистеми: щоб "Sidebar передає дані в Business", "Smart Capture
відправляє в Creator", AI Compare/Site Inspector формують звіти в
Business — усі чотири продукти (Business, Mail, Creator, Office)
мають вже існувати з реальними API для прийому даних ззовні. Той
самий принцип залежності знизу вгору, що вже застосований у хвилі 4
цього документа (Predictive AI/Benchmarking чекають на Knowledge
Graph/Team Workspace) і в розділі Creator (Live Objects чекають на
`live`-модулі Business).

### Що свідомо НЕ робити на старті

За аналогією з явними застереженнями Артема щодо Office
(не намагатись замінити Microsoft Office) і Canvas Core в Creator
(не будувати рушій нескінченного полотна з нуля) — тут той самий
принцип масштабу: не намагатись одразу побудувати повноцінний
браузерний рушій чи повторити всі 20+ фіч вище одночасно. З переліку
найреалістичніший технічний шлях для MVP, коли дійде черга:
- побудова на готовому Chromium-based рушії (Electron/CEF або
  подібне), не написання рендерера з нуля — прямий аналог рішення
  "не будувати Canvas Core з нуля" в Creator;
- AI Sidebar + Site Inspector — найдешевші й найочевидніші кандидати
  на перший видимий результат (не вимагають нової інфраструктури,
  лише виклик вже наявних Gemini-хелперів на вміст сторінки);
- Component Extractor і Research Mode — найризикованіші технічно і
  юридично (копірайт чужого контенту), відкладаються на пізніше і
  вимагають окремого обговорення меж дозволеного.

**Наступний крок, коли дійде черга (не зараз):** вирішити з Артемом
базову технічну платформу (Electron/CEF/інше), і чи Browser
реалістичний як самостійний продукт для соло-розробника, чи вимагає
окремої команди — за масштабом ідеї це найбільший з п'яти продуктів
екосистеми.
