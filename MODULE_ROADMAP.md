...
## Qorax Office — окремий продукт екосистеми (концепція, не модуль платформи)

**Джерело:** продуктове бачення від Артема, отримано як текстовий
документ з повним описом фіч. **Статус: зафіксовано як напрямок на
майбутнє, БЕЗ коду** — жодна реалізація цим проходом не починалась,
нижче лише структурований запис концепції, щоб не загубити її між
сесіями.
...
окремий продукт зі своєю точкою входу — окремий домен/піддомен
(наприклад `office.qorax-sites.com`), окремий лендинг, окремий вхід.
Технічно це означає окремий Next.js/Worker деплой у довгостроковій
перспективі, не нову директорію під `app/dashboard/`.
...
**Наступний крок, коли дійде черга (не зараз):** обговорити з Артемом
технічну реалізацію окремого домену/деплою (Cloudflare Worker+Pages
проєкт окремо від `qorax`/`qorax-api`, чи monorepo з спільними
пакетами) і вибрати MVP-підмножину з переліку продуктів вище.
